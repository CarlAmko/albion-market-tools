...
def get_rune_prices(tier: int, town: str):
    ...
    if _prefix:
        ...
        return datafetch.get_current_prices(rune_id, town=town)

    return None


def get_soul_prices(tier: int, town: str):
    def create_soul_name(pre: str):
        return pre + "'s Soul"

    _prefix = prefix.TIER_PREFIXES[tier]
    if _prefix:
        rune_id = datafetch.find_item_id_by_name(create_soul_name(_prefix))
        return datafetch.get_current_prices(rune_id, town=town)

    return None


def get_relic_prices(tier: int, town: str):
    def create_relic_name(pre: str):
        return pre + "'s Relic"

    _prefix = prefix.TIER_PREFIXES[tier]
    if _prefix:
        rune_id = datafetch.find_item_id_by_name(create_relic_name(_prefix))
        return datafetch.get_current_prices(rune_id, town=town)

    return None


def calculate_profits_for_item(item_id: str, town: str):
    ...
    for item_id in all_item_ids:
        prices.append(datafetch.get_current_prices(item_id, town=town))

    profits = {}
    for i in range(len(prices) - 1):
        base_profit = prices[i + 1]['sell_price_min'] - prices[i]['sell_price_min']
        tier = prefix.get_tier(item_id)

        # X.0 -> X.1
        if i == 0:
            enchant_cost = get_rune_prices(tier, town=town)
        # X.1 -> X.2
        elif i == 1:
            enchant_cost = get_soul_prices(tier, town=town)
        # X.2 -> X.3
        elif i == 2:
            enchant_cost = get_relic_prices(tier, town=town)
        else:
...
def find_best_enchants(town: str, limit: int = 20):
    ...
    for item in datafetch.ITEM_DATA:
        ...
        if '@' not in unique_name:
            calculate_profits_for_item(unique_name, town=town)
